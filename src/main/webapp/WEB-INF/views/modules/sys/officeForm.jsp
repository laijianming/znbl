...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/sys/office/form?id=${office.id}&parent.id=${officeparent.id}">机构添加<shiro:hasPermission name="sys:office:edit">${not empty office.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="sys:office:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
		...
		<div class="control-group">
			...
			<div class="controls">
				<sys:treeselect id="parent" name="parent.id" value="${office.parent.id}" labelName="parent.name" labelValue="${office.parent.name}"
					title="父级编号" url="/sys/office/treeData" extId="${office.id}" cssClass="" allowClear="true"/>
			</div>
		</div>
		<div class="control-group">
