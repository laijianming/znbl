...
import com.znbl.modules.sys.utils.LogUtils;
...
public class SystemService extends BaseService {
	...
	//
	// for(User user1 : list){
	// user1.setRoleList(roleDao.findList(new Role(user1)));
	// }
	//
	// // 执行分页查询
	// page.setList(list);
	...
	// if (StringUtils.isBlank(user.getId()) || userDao.get(user.getId()) ==
	// null){
	...
	/*
	 * @Transactional(readOnly = false) public void saveRole(Role role) { if
	 * (StringUtils.isBlank(role.getId())){ role.preInsert();
	 * roleDao.insert(role); }else{ role.preUpdate(); roleDao.update(role); } //
	 * 更新角色与菜单关联 roleDao.deleteRoleMenu(role); if (role.getMenuList().size() >
	 * 0){ roleDao.insertRoleMenu(role); }
	 * 
	 * // 清除用户角色缓存 UserUtils.removeCache(UserUtils.CACHE_ROLE_LIST); // //
	 * 清除权限缓存 // systemRealm.clearAllCachedAuthorizationInfo(); }
	 */
